...
Coord3d rotate(Coord3d a, double x, double y, double z) {
    Coord3d mat[] = {
        (Coord3d){cos(z)*cos(y), cos(z)*sin(y)*sin(x) - sin(z)*cos(x), cos(
        (Coord3d){sin(z)*cos(y), sin(z)*sin(y)*sin(x) + cos(z)*cos(x), sin(
...
void oval(double a, double b) {
    glBegin(GL_POLYGON);
    for (int i=0; i<100; i++) {
        glVertex3d(0, a * cos(M_PI * 2 * i / 100), b * sin(M_PI * 2 * i / 1
    }
    glEnd();
}
...
void face(void) {
    ...
    glBegin(GL_LINE_STRIP);
    for (int i=-40; i<=40; i++) {
        Coord3d new = rotate((Coord3d){0.93,0,0}, 0, sqrt(45*45-i*i)*M_PI/1
        glVertex3d(new.x, new.y, new.z);
    }
    glEnd();
    glPopMatrix();
...
void leg_rec(double from, double to, int steps, double length) {
    ...
    for (int i=steps; i>0; i--) {
        ...
        for (int j=30; j>=0; j--) {
        glColor3fv(blue);
            glVertex3d(radius * cos(M_PI * j / 15), radius * sin(M_PI * j /
            glVertex3d(next_r * cos(M_PI * j / 15), next_r * sin(M_PI * j /
        }
        glEnd();
...
void front_back_rec(double from, double to, int steps, double length, doubl
    glPushMatrix();
    int cumul = steps * (steps + 1) / 2;
    double delta_r = (to - from) / cumul;
    double delta_z = length / steps;
